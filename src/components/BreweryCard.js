...
const BreweryCard = ({ brewery, getBrewery, addFavoriteBrewery, user, removeFavoriteBrewery }) => {
  return (
    <div className='beer-card'>
      ...
      { removeFavoriteBrewery ? <h4 onClick={() => removeFavoriteBrewery(user.id, brewery.firebaseID)}>UNFAVORITE</h4> : <h4 onClick={() => addFavoriteBrewery(user.id, 'brewery', brewery.id, brewery)}>FAVORITE</h4> }
    </div>
  );
};
